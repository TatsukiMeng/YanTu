...
- [x] 1.1 创建根 `package.json` 配置 npm workspaces
  - **写入范围**: 根目录 `package.json`
  - **验证**: `npm ls --workspaces` 显示 miniapp + server
  - **依赖**: 无

- [x] 1.2 创建 `packages/miniapp/`，迁移现有小程序文件
...
- [x] 1.5 创建 `packages/server/` 目录，初始化项目
...
- [x] 2.1 创建 `packages/server/src/db.ts`
...
- [x] 3.1 创建 `packages/miniapp/utils/api.js`
...
- [x] 3.2 创建 `packages/miniapp/utils/auth.js`
